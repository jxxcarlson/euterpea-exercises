...
data Grammar a = Grammar  a
                         (Rules a)
    deriving Show

data Rules a = Uni [Rule a] | Sto [(Rule a, Prob)]
...
gen f (Grammar s rules) seed =
    ...
    in if checkProbs newRules
       ...
       else (error "Stochastic rule-set is malformeed.")
...
interpet (a :. b) r m = interpret a r (interpret b r m)
interpet (a :+ b) r m = interpret a r m :+: interpret b r m
interpret Id      r m = m
...
t1 n = instrument Vibraphone $ interpret (gen replFun g1 42 !! n) ir (c 5 tn)
